...
> 개발자에게 모나드는 신비주의에 가깝다. 모나드는 여러가지 컨셉을 추상화 한 까닭에 일상에서 그 뜻을 유추하기 어렵다. 하지만 모나드는 실제로 간단한 컨셉이며 그 다양한 적용처 때문에 한마디로 정의하기 어려워 혼동될 뿐이다. 절차적인 방식의 프로그래밍 배경이 있는 사람은 이해하기 어렵다. 문제는 우리가 프로그래밍을 함수의 합성으로 바라보지 않는데 있다.
...
> 보편적인 클레이슬리 카테고리의 정의: 본래 카테고리 C에 대하여 클레이슬리 카테고리 K가 있을 때, K의 오브젝트는 C와 동일하되 모피즘은 `a -> b` 대신 `a -> m b`와 같다. 여기서 m은 endofunctor이다.
...
> 클레이슬리 합성을 이용한 모나드의 정의. 이것이 모나드를 정의하는 유일한 방법은 아니다.
...
> 하스켈에서 모나드 법칙. 이 표현식을 보면 모나드가 embellished function을 합성하는 방법이라는 것을 알 수 있다. 이 과정에서 사이드이펙트나 상태가 존재하지 않지만, embelished function이 그것들을 표현하느데 사용되기도 하므로 모나드가 이를 다루는데 사용될 수 있나보다?
